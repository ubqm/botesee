...
from application.clients.models.faceit.match_stats import MatchStatistics, Round, Player
from application.discord_bot.models.embed import NickEloStorage, PlayerStorage
from application.tests.conftest import match_finished
from utils.enums import available_maps, colors
from application.web.models.events import MatchFinished
...
class MatchFinishedImCol:
    ...
    async def _get_background(self, round_: Round) -> Image:
        if round_.round_stats.map not in available_maps:
            return Image.open(f"../../templates/maps/black.jpg")

        image_map = Image.open(f"../templates/maps/{round_.round_stats.map}.jpg")
