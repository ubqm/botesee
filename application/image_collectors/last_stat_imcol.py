...
from application.clients.models.faceit.match_stats import Round, MatchStatistics
from application.clients.models.faceit.player_history import MatchHistory
from application.clients.steam import SteamClient
from application.image_collectors.models.last_stat import GameStatLast, SteamStatLast, GameStatLastStorage, FullPlayerStat
from utils.enums import colors


class LastStatsImCol:
    font = ImageFont.truetype("../templates/fonts/Outfit/Outfit-Bold.ttf", 26)
    font_name = ImageFont.truetype("../templates/fonts/Outfit/Outfit-Bold.ttf", 36)
    lose_bg = Image.open("../../templates/background_features/right-side-lose.png")
    win_bg = Image.open("../../templates/background_features/right-side-win.png")
    bg_dark_right = Image.open(f"../../templates/background_features/dark-right-side-for-stat.png")
    ...
    async def _set_avatar(self, session: ClientSession) -> None:
        if self.player_stat[self.nickname].player_details.avatar:
            ...
            self.image_avatar = Image.open("../../templates/question-mark-icon.jpg")
            self.image_avatar = self.image_avatar.resize((130, 130))
    ...
    def _draw_game_map(self, canvas: ImageDraw, game: GameStatLast, idx_game: int) -> None:
        ...
        current_map = Image.open(f"../templates/maps/{game.map_name}.jpg")
