...
class ImageCollectorStatLast:
    ...
    async def get_player_info(session, nickname):
        ...
        if pd is not None:
            ...
            if history is not None:
                region_place = await region_stats(
                    session=session,
                    player_id=pd["player_id"],
                    region=pd["games"]["csgo"]["region"],
                )
                region_place = region_place["items"][0]["position"]
                country_place = await region_stats(
                    session=session,
                    player_id=pd["player_id"],
                    region=pd["games"]["csgo"]["region"],
                    country=pd["country"],
    ...
    async def draw_image(player_stat):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            rec_pl_stat = await user_rec_played_stat(
                session, player_stat[0]["steam_id"]
            )
        ...
        if (
            "response" in rec_pl_stat.keys()
            and "games" in rec_pl_stat["response"].keys()
        ):
            for app in rec_pl_stat["response"]["games"]:
                if app["appid"] == 730:
                    playtime_2weeks = (
                        f"Last 2 weeks: {int(app['playtime_2weeks'] / 60)} hrs"
                    )
                    playtime_forever = (
                        f"Summary in CSGO: {int(app['playtime_forever'] / 60)} hrs"
                    )
                    if app_stat is not None:
                        csgo_playtime = (
                            app_stat["playerstats"]["stats"][2]["value"] / 60 / 60
                        )
                        csgotime_played_hrs = (
                            f"Played in CSGO: {int(csgo_playtime)} hrs"
                        )
                        percentage_played = (
                            f"Activity: "
        ...
        image_lvl = Image.open(f"templates/faceit_icons/faceit{faceitlvl}.png").convert(
            "RGBA"
        )
        ...
        for i in range(10):
            ...
            draw_image_bg.text(
                (870, 50 * i + 30), player_stat[i]["mapscore"], font=font
            )
            ...
            kad = (
                f"{player_stat[i]['kills']}/"
                f"{player_stat[i]['assists']}/"
                f"{player_stat[i]['deaths']}"
            )
            draw_image_bg.text((665, 50 * i + 30), kad, font=font, fill=stat_color)
            dd = datetime.utcfromtimestamp(player_stat[i]["started_at"]).strftime(
                "%m/%d"
            )
            cc = calendar.month_name[int(dd.split("/")[0])][:3]
            draw_image_bg.text(
                (570, 50 * i + 30), dd.split("/")[1] + " " + cc, font=font
            )

        mean_k, mean_a, mean_d, mean_kd, mean_kr, mean_hs = map(
            (lambda x: x / 10), [mean_k, mean_a, mean_d, mean_kd, mean_kr, mean_hs]
        )

        draw_image_bg.text((10, 310), "Last 10 games played:", font=font)
        draw_image_bg.text(
            (10, 340),
            f"KAD: {mean_k:.1f} / " f"{mean_a:.1f} / " f"{mean_d:.1f}",
            font=font,
        )
        ...
        draw_image_bg.text(
            (270, 70),
            f"{player_stat[0]['region']}: " f"{player_stat[0]['region_place']}",
            font=font,
        )
        draw_image_bg.text(
            (270, 100),
            f"{player_stat[0]['country']}: " f"{player_stat[0]['country_place']}",
            font=font,
        )
