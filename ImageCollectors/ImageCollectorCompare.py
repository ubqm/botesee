...
from ImageCollectors.enums import AvailableMaps
from api_funcs.async_faceit_get_funcs import (
    player_details,
    player_history,
    match_stats,
    region_stats,
...
class ImageCollectorCompare:
    def __init__(
        ...
        self.font_name = ImageFont.truetype(
            f"templates/fonts/Outfit/Outfit-Bold.ttf", 36
        )
    ...
    async def get_player_info(self, session, nickname):
        ...
        if pd is not None:
            history = await player_history(
                session, pd["player_id"], offset=0, limit=self.amount
            )
    ...
    def get_comparison_dict(self, player1_stats, player2_stats):
        comparison_dict = {
            ...
            "skill_level": (
                player1_stats[0]["skill_level"],
                player2_stats[0]["skill_level"],
            ),
            "faceit_elo": (
                player1_stats[0]["faceit_elo"],
    ...
    def get_mean_stats(self, player_stat):
        mean_k = (
            ...
            mean_d
        ) = mean_kd = mean_kr = mean_hs = total_4k = total_5k = winrate = mvps = 0
        map_dict = {}
        for single_map in AvailableMaps:
        ...
        for i in range(min(int(self.amount), len(player_stat))):
            ...
            if player_stat[i]["mapname"] in AvailableMaps:
                map_dict[player_stat[i]["mapname"]][0] += 1
                if player_stat[i]["result"] == "1":
                    map_dict[player_stat[i]["mapname"]][1] += 1

        mean_k, mean_a, mean_d, mean_kd, mean_kr, mean_hs, winrate, mvps = map(
            (lambda x: x / len(player_stat)),
    ...
    def place_stat_map(self, comparison_dict):
        ...
        right_stat_x = 850
        i = 0
        for enum_obj in AvailableMaps:
            available_map = enum_obj.value
            if comparison_dict[available_map][0][1] != 0:
                left_line = (
                    f"{comparison_dict[available_map][0][1]} / {comparison_dict[available_map][0][0]} — "
                    f"{int(comparison_dict[available_map][0][1] / comparison_dict[available_map][0][0] * 100)}%"
                )
            else:
                left_line = (
                    f"{comparison_dict[available_map][0][1]} / {comparison_dict[available_map][0][0]} — 0%"
                )
            if comparison_dict[available_map][1][1] != 0:
                right_line = (
                    f"{int(comparison_dict[available_map][1][1] / comparison_dict[available_map][1][0] * 100)}% — "
                    f"{comparison_dict[available_map][1][1]} / {comparison_dict[available_map][1][0]}"
                )
            else:
                right_line = (
                    f"0% — {comparison_dict[available_map][1][1]} / {comparison_dict[available_map][1][0]}"
                )

            self.draw_bg.text(
                (left_stat_x, map_h * i + map_y_start + 10), left_line, font=self.font
            )
            w, h = self.draw_bg.textsize(right_line, font=self.font)
            self.draw_bg.text(
                (right_stat_x - w, map_h * i + map_y_start + 10),
                right_line,
                font=self.font,
            )
    ...
    def get_player_background(bg_link, output_width=960, output_height=540):
        if bg_link != "":
            ...
            background = Image.open(
                f"templates/background_features/dark-right-side-for-stat.png"
            )
    ...
    def draw_image(self, player1_stats, player2_stats):
        ...
        dark_middle = Image.open(
            f"templates/background_features/dark-middle-compare.png"
        )
        player1_bg = self.get_player_background(
            player1_stats[0]["background"], 480, 540
        )
        player2_bg = self.get_player_background(
            player2_stats[0]["background"], 480, 540
        )
        ...
        self.draw_bg.text(
            (810 - w, 20), player2_stats[0]["nickname"], font=self.font_name
        )

        comparison_dict = self.get_comparison_dict(player1_stats, player2_stats)

        w, h = self.draw_bg.textsize(
            f"{self.amount} {self.output_type}", font=self.font
        )
        self.draw_bg.text(
        ...
        self.draw_bg.text(
            (160, 70),
            f"{comparison_dict['region'][0]}: {comparison_dict['region_place'][0]}",
            font=self.font,
        )
        self.draw_bg.text(
            (160, 100),
            f"{comparison_dict['country'][0]}: {comparison_dict['country_place'][0]}",
            font=self.font,
        )
        ...
        w, h = self.draw_bg.textsize(
            f"{comparison_dict['region'][1]}: {comparison_dict['region_place'][1]}",
            font=self.font,
        )
        self.draw_bg.text(
            (810 - w, 70),
        ...
        w, h = self.draw_bg.textsize(
            f"{comparison_dict['country'][1]}: {comparison_dict['country_place'][1]}",
            font=self.font,
        )
        self.draw_bg.text(
            (810 - w, 100),
