...
from ImageCollectors.enums import ColorTuple, AvailableMaps
...
class ImageCollectorMatchFinished:
    def __init__(
        ...
        self.stat_color = ColorTuple.WHITE
        ...
        self.image_dark_avatar_bot = Image.open(
            "templates/background_features/for_avatar_bot.png"
        )
        self.image_dark_avatar_top = Image.open(
            "templates/background_features/for_avatar_top.png"
        )
        self.image_dark_middle = Image.open(
            "templates/background_features/dark-middle2.png"
        )
                    ...
                    for idx_player, player in enumerate(team["players"]):
                        task = asyncio.create_task(
                            self.draw_player_on_image(
                                session, player, idx_team, idx_player
                            )
                        )
                        tasks.append(task)
    ...
    def bool_overtime_check(match):
        if (
            "Overtime score" in match["teams"][0]["team_stats"].keys()
            and match["teams"][0]["team_stats"]["Overtime score"] == "0"
            and match["teams"][1]["team_stats"]["Overtime score"] == "0"
        ):
            return False
        else:
            return True
    ...
    def draw_map_image(self, match):
        if match["round_stats"]["Map"] in AvailableMaps:
            self.image_map = Image.open(
                f"templates/maps/{match['round_stats']['Map']}.jpg"
            )
        ...
        if match["teams"][0]["team_stats"]["Team Win"] == "1":
            ...
            image_botcolor = Image.open(
                "templates/background_features/Lose-botleft.png"
            )
        else:
            image_topcolor = Image.open(
                "templates/background_features/Lose-topleft.png"
            )
        ...
        for idx_team, team in enumerate(match["teams"]):
            if is_overtime_in_match:
                halftimes = (
                    f"{team['team_stats']['First Half Score']}"
    ...
    def get_fonts(font_folder, font_file, font_file_mainscore):
        fonts = {
            "mainscore": ImageFont.truetype(
                f"templates/fonts/{font_folder}/{font_file_mainscore}", 50
            ),
            "avatar": ImageFont.truetype(
                f"templates/fonts/{font_folder}/{font_file}", 18
            ),
            "player_score": ImageFont.truetype(
                f"templates/fonts/{font_folder}/{font_file}", 32
            ),
            "player_stats": ImageFont.truetype(
                f"templates/fonts/{font_folder}/{font_file}", 22
            ),
    ...
    async def draw_avatar_on_image(self, session, player, idx_team, idx_player):
        ...
        player_elo = str(
            player_elo["games"]["csgo"]["faceit_elo"] if player_elo else "N/A"
        )
        ...
        for idx_req_player, req_player in enumerate(
            ...
            if player["nickname"] == req_player["nickname"]:
                ...
                if idx_team == 0:
                    ...
                    image_avatar.paste(
                        Image.open(
                            f"templates/faceit_icons/faceit{faceitlvl}.png"
                        ).convert("RGBA"),
                        (0, 0),
                        Image.open(
                            f"templates/faceit_icons/faceit{faceitlvl}.png"
                    ...
                    w, h = self.draw_image_map.textsize(
                        elo_diff, font=self._fonts["avatar"]
                    )
                    draw_image_avatar.text(
                        (127 - w, 0), elo_diff, font=self._fonts["avatar"]
                    )
                    # w, h = self.draw_image_map.textsize(player_elo, font=self._fonts['avatar'])
                    draw_image_avatar.text(
                        (26, 0), player_elo, font=self._fonts["avatar"]
                    )
                else:
                    image_avatar.paste(
                        self.image_dark_avatar_top, (0, 0), self.image_dark_avatar_top
                    ...
                    draw_image_avatar.text(
                        (127 - w, 107), elo_diff, font=self._fonts["avatar"]
                    )
                    # w, h = self.draw_image_map.textsize(player_elo, font=self._fonts['avatar'])
                    draw_image_avatar.text(
                        (26, 107), player_elo, font=self._fonts["avatar"]
                    )

                w, h = self.draw_image_map.textsize(
                    req_player["nickname"], font=self._fonts["avatar"]
                )
                if w > 130:
                    draw_image_avatar.text(
                        (0, 107 - idx_team * 107),
    ...
    def draw_player_stats_on_image(self, player, idx_team, idx_player):
        if float(player["player_stats"]["K/D Ratio"]) >= 1.3:
            self.stat_color = ColorTuple.GREEN
        elif float(player["player_stats"]["K/D Ratio"]) < 0.6:
            self.stat_color = ColorTuple.RED
        elif 0.8 > float(player["player_stats"]["K/D Ratio"]) >= 0.6:
            self.stat_color = ColorTuple.ORANGE
        else:
            self.stat_color = ColorTuple.WHITE

        kad = (
            f"{player['player_stats']['Kills']}"
        ...
        self.draw_image_map.text(
            (156 + idx_player * 162, 240 + 36 * idx_team),
            mvp,
            font=self._fonts["player_stats"],
        )
        kr = f"K/R: {player['player_stats']['K/R Ratio']}"
        self.draw_image_map.text(
            (156 + idx_player * 162, 220 + 76 * idx_team),
            kr,
            font=self._fonts["player_stats"],
        )
